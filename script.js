...
function animate() {
    // 1. Dibujar el fondo fluido (Satén animado por gradiente oscilante)
    const time = Date.now() * 0.0002;
    
    // Tonos oscilantes para simular el movimiento de la seda bajo la luz
    const colorStop1 = `hsl(335, 100%, ${10 + Math.sin(time) * 5}%)`; 
    const colorStop2 = `hsl(340, 100%, ${5 + Math.cos(time * 0.8) * 3}%)`; 
    
    // Gradiente dinámico radial que se desplaza sutilmente
